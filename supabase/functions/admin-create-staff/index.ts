...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: staffRow, error: staffErr } = await adminPublic.rpc("create_staff_member", {
    ...
    if (staffErr) {
      ...
      throw new Error(`Staff insert failed: ${staffErr.message}`);
    }

    return json({ staff: Array.isArray(staffRow) ? staffRow[0] : staffRow });
  } catch (err: any) {
    console.error("createStaff failed:", err);
    return json({ error: err?.message || "Could not create staff account" }, 500);
  }
});
